  ...
  d3el.selectAll('li')
    .on('click', function () {
      ...
      if (containerExpanded) {
        ...
        let menuBBox = {};
        d3el.selectAll('li').each(function () {
          // this refers to the DOM element
          let temp = this.getBoundingClientRect();
          menuBBox.left = menuBBox.left === undefined ||
            temp.left < menuBBox.left ? temp.left : menuBBox.left;
          menuBBox.top = menuBBox.top === undefined ||
            temp.top < menuBBox.top ? temp.top : menuBBox.top;
          menuBBox.right = menuBBox.right === undefined ||
            temp.right > menuBBox.right ? temp.right : menuBBox.right;
          menuBBox.bottom = menuBBox.bottom === undefined ||
            temp.bottom > menuBBox.bottom ? temp.bottom : menuBBox.bottom;
        });
